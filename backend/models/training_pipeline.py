...
def discover_non_transfers(
    ...
    exclude_player_seasons : set[tuple[int, int]], optional
        (player_id, season_id) pairs to skip (e.g. players already in
        transfer records for a specific season) to prevent data leakage.
    min_minutes : int
            ...
            for player in players:
                    ...
                    for t in transfers:
                        ...
                        if t_date:
                            ...
                            if t_year in relevant_years:
                                moved = True
                                break
                        # else: undated transfer — skip it (don't assume moved)

                # Additional team_id cross-check: verify the player's team_id
                # in the post season matches their pre-season team_id.
                # Also doubles as the post-season minutes check to avoid
                ...
                if not moved:
                    ...
                    if pre_team_id:
                        ...
                        if post_team_id and post_team_id != pre_team_id:
                            moved = True
                ...
                # Verify minutes in post season (reuses post_stats from above)
...
def run_pipeline(
    *,
    ...
    if not skip_build:
        ...
        # Build (player_id, season_id) pairs from transfer records for exclusion
        transfer_player_seasons = set()
        for r in records:
            transfer_player_seasons.add((r.player_id, r.pre_season_id))
            transfer_player_seasons.add((r.player_id, r.post_season_id))
