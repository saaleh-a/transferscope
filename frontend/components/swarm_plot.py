...
def show_swarm_grid(
    ...
    for i, metric in enumerate(metrics):
        player_val = v if (v := player_per90.get(metric)) is not None else 0
        tm_vals = [v if (v := d.get(metric)) is not None else 0 for d in teammate_per90s if d.get(metric) is not None]
        lg_vals = [v if (v := d.get(metric)) is not None else 0 for d in league_per90s if d.get(metric) is not None]
